...
export default function EditBillingPage() {
  ...
  useEffect(() => {
    ...
    try {
      const saved = JSON.parse(localStorage.getItem("hs_billing") || "null");
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    localStorage.setItem("hs_billing", JSON.stringify(form));
  ...
  return (
    <div className="hs-container" style={{ padding: "16px" }}>
      <p style={{ fontSize: 12, color: "#666", marginBottom: 12 }}>
        ...
        <Link href="/account" style={{ color: "var(--red)" }}>My Account</Link> &gt; Edit Billing Address
      </p>
      <h1 className="page-heading">Edit Billing Address</h1>
